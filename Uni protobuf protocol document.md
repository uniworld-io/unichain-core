...
This is the description of  Google Protobuf implementation of Uni's protocol.
...
- Uni has 3 `types` of account: Normal, AssetIssue, Contract
...
- message `Account` has multiple attributes and 2 nested messages: 
  ...
  `balance`: the UNW balance of this account.

  `votes`: received votes of this account. – e.g. *{(“0x1b7w…9xj3”,323), (“0x8djq…j12m”,88),…,(“0x82nd…mx6i”,10001)}*.

  `asset`: other assets except UNW in this account – e.g. *{<“WishToken”,66666>,<”Dogie”,233>}*.

  `assetV2`: other assets except UNW in this account – e.g. *{<“WishToken”,66666>,<”Dogie”,233>}*. (used after allowing same name of token87)

  `frozen`: the freezed UNW of this account for receiving bandwidth
...
- message `TransactionRet`
  ...
  - message `ResourceReceipt`
    ...
    `net_fee`: consume yourself unw of net usage.
  ...
  - Message `InternalTransaction`
    ...
    `callValueInfo`: Refers to asset transfer information in internal transactions, including unw and urc10.
  ...
  - message `TransactionInfo`
   ...
   `unfreeze_amount`: unfreeze unw amount.
...
- Uni has 33 types of Contracts declared within [`Transaction`](#trans).
...
- There are 15 types of results while deploying contracts (refer to `Transaction.Result`):
  ...
  - message `VoteWitnessContract`
    ...
    - message `AssetIssueContract`
      ...
      `unx_num`: Unx num defines token price.

      `precision`: precision.

      `num`: Unx num defines token price.
    ...
    - message `FreezeBalanceContract`
      ...
      `frozen_balance`: frozen amount of UNW.

      `frozen_duration`: frozen duration of UNW.

      `resource`: type of resource gained from freezing UNW.
    ...
    - message `UnfreezeBalanceContract`
     ...
     - message `CreateSmartContract`
       ...
       `call_token_value`: amount of URC10 token sent to the newly created smart contract.
    
       `token_id`: URC10 token id.
     ...
     - message `TriggerSmartContract`
       ...
       `call_value`: UNW amount sent to smart contract.
    
       `data`: functions and parameters called in smart contract.
    
       `call_token_value`: URC10 token amount sent to smart contract.
...
- message `SmartContract`
  ...
  `call_value`: amount of UNW that send to the smart contract.
  ...
  `unx_hash`:  transactionId of Deploying contract transaction.
