...
public class FreezeBalanceActuator extends AbstractActuator {
  ...
  public boolean execute(Object result) throws ContractExeException {
    ...
    switch (freezeBalanceContract.getResource()) {
      case BANDWIDTH:
        ...
        dynamicStore
            .addTotalNetWeight(frozenBalance / UNW_PRECISION);
        break;
      case ENERGY:
        if (!ArrayUtils.isEmpty(receiverAddress)
            && dynamicStore.supportDR()) {
        ...
        dynamicStore
            .addTotalEnergyWeight(frozenBalance / UNW_PRECISION);
        break;
      case UNX_POWER:
        long newFrozenBalanceForUnxPower =
            frozenBalance + accountCapsule.getUnwPowerFrozenBalance();
        accountCapsule.setFrozenForUnwPower(newFrozenBalanceForUnxPower, expireTime);

        dynamicStore
            .addTotalUnxPowerWeight(frozenBalance / UNW_PRECISION);
        break;
      default:
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (frozenBalance < UNW_PRECISION) {
      throw new ContractValidateException("frozenBalance must be more than 1UNW");
    }
...
//    }
    ...
    switch (freezeBalanceContract.getResource()) {
      ...
      case UNX_POWER:
        if (dynamicStore.supportAllowNewResourceModel()) {
          ...
          if (!ArrayUtils.isEmpty(receiverAddress)) {
            throw new ContractValidateException(
                "UNX_POWER is not allowed to delegate to other accounts.");
          }
        } else {
          throw new ContractValidateException(
    ...
    if (!ArrayUtils.isEmpty(receiverAddress) && dynamicStore.supportDR()) {
      ...
      if (dynamicStore.getAllowUvmConstantinople() == 1
