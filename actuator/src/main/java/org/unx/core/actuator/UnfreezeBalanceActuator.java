...
public class UnfreezeBalanceActuator extends AbstractActuator {
  ...
  public boolean execute(Object result) throws ContractExeException {
    ...
    if (!ArrayUtils.isEmpty(receiverAddress) && dynamicStore.supportDR()) {
      ...
      if (dynamicStore.getAllowUvmConstantinople() == 0 ||
          (receiverCapsule != null && receiverCapsule.getType() != AccountType.Contract)) {
        switch (unfreezeBalanceContract.getResource()) {
          case BANDWIDTH:
            if (dynamicStore.getAllowUvmSolidity059() == 1
      ...
      switch (unfreezeBalanceContract.getResource()) {
        ...
        case ENERGY:
          ...
          break;
        case UNX_POWER:
          unfreezeBalance = accountCapsule.getUnwPowerFrozenBalance();
          accountCapsule.setInstance(accountCapsule.getInstance().toBuilder()
              .setBalance(oldBalance + unfreezeBalance)
              .clearUnwPower().build());
          break;
    ...
    switch (unfreezeBalanceContract.getResource()) {
      case BANDWIDTH:
        dynamicStore
            .addTotalNetWeight(-unfreezeBalance / UNW_PRECISION);
        break;
      case ENERGY:
        dynamicStore
            .addTotalEnergyWeight(-unfreezeBalance / UNW_PRECISION);
        break;
      case UNX_POWER:
        dynamicStore
            .addTotalUnxPowerWeight(-unfreezeBalance / UNW_PRECISION);
        break;
      default:
        //this should never happen
        break;
    ...
    if (dynamicStore.supportAllowNewResourceModel()
        && accountCapsule.oldUnwPowerIsInvalid()) {
      switch (unfreezeBalanceContract.getResource()) {
        case BANDWIDTH:
        case ENERGY:
    ...
    if (dynamicStore.supportAllowNewResourceModel()
        && !accountCapsule.oldUnwPowerIsInvalid()) {
      accountCapsule.invalidateOldUnwPower();
    }
  ...
  public boolean validate() throws ContractValidateException {
    ...
    if (!ArrayUtils.isEmpty(receiverAddress) && dynamicStore.supportDR()) {
      ...
      switch (unfreezeBalanceContract.getResource()) {
        ...
        case UNX_POWER:
          if (dynamicStore.supportAllowNewResourceModel()) {
            Frozen frozenBalanceForUnxPower = accountCapsule.getInstance().getUnwPower();
            if (frozenBalanceForUnxPower.getFrozenBalance() <= 0) {
              throw new ContractValidateException("no frozenBalance(UnxPower)");
            }
            if (frozenBalanceForUnxPower.getExpireTime() > now) {
              throw new ContractValidateException("It's not time to unfreeze(UnxPower).");
            }
          } else {
            throw new ContractValidateException(
                "ResourceCode error.valid ResourceCode[BANDWIDTH、Energy]");
        ...
        default:
          if (dynamicStore.supportAllowNewResourceModel()) {
            throw new ContractValidateException(
                "ResourceCode error.valid ResourceCode[BANDWIDTH、ENERGY、UNX_POWER]");
          } else {
            throw new ContractValidateException(
                "ResourceCode error.valid ResourceCode[BANDWIDTH、ENERGY]");
          }
