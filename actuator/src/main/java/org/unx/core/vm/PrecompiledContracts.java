...
import static org.unx.common.runtime.vm.DataWord.WORD_SIZE;
import static org.unx.common.utils.BIUtil.addSafely;
import static org.unx.common.utils.BIUtil.isLessThan;
import static org.unx.common.utils.BIUtil.isZero;
import static org.unx.common.utils.ByteUtil.EMPTY_BYTE_ARRAY;
import static org.unx.common.utils.ByteUtil.bytesToBigInteger;
import static org.unx.common.utils.ByteUtil.longTo32Bytes;
import static org.unx.common.utils.ByteUtil.merge;
import static org.unx.common.utils.ByteUtil.numberOfLeadingZeros;
import static org.unx.common.utils.ByteUtil.parseBytes;
import static org.unx.common.utils.ByteUtil.parseWord;
...
import org.unx.common.crypto.Blake2bfMessageDigest;
import org.unx.common.crypto.Hash;
import org.unx.common.crypto.SignUtils;
import org.unx.common.crypto.SignatureInterface;
import org.unx.common.crypto.zksnark.BN128;
import org.unx.common.crypto.zksnark.BN128Fp;
import org.unx.common.crypto.zksnark.BN128G1;
import org.unx.common.crypto.zksnark.BN128G2;
import org.unx.common.crypto.zksnark.Fp;
import org.unx.common.crypto.zksnark.PairingCheck;
...
public class PrecompiledContracts {
  ...
  public static PrecompiledContract getContractForAddress(DataWord address) {
    ...
    if (VMConfig.allowUvmSolidity059() && address.equals(batchValidateSignAddr)) {
      return batchValidateSign;
    }
    if (VMConfig.allowUvmSolidity059() && address.equals(validateMultiSignAddr)) {
      return validateMultiSign;
    }
    if (VMConfig.allowShieldedURC20Transaction() && address.equals(verifyMintProofAddr)) {
      return verifyMintProof;
    }
    if (VMConfig.allowShieldedURC20Transaction() && address.equals(verifyTransferProofAddr)) {
      return verifyTransferProof;
    }
    if (VMConfig.allowShieldedURC20Transaction() && address.equals(verifyBurnProofAddr)) {
      return verifyBurnProof;
    }
    if (VMConfig.allowShieldedURC20Transaction() && address.equals(merkleHashAddr)) {
      return merkleHash;
    }
    if (VMConfig.allowUvmVote() && address.equals(rewardBalanceAddr)) {
      return rewardBalance;
    }
    if (VMConfig.allowUvmVote() && address.equals(isSrCandidateAddr)) {
      return isSrCandidate;
    }
    if (VMConfig.allowUvmVote() && address.equals(voteCountAddr)) {
      return voteCount;
    }
    if (VMConfig.allowUvmVote() && address.equals(usedVoteCountAddr)) {
      return usedVoteCount;
    }
    if (VMConfig.allowUvmVote() && address.equals(receivedVoteCountAddr)) {
      return receivedVoteCount;
    }
    if (VMConfig.allowUvmVote() && address.equals(totalVoteCountAddr)) {
      return totalVoteCount;
    }
    if (VMConfig.allowUvmCompatibleEvm() && address.equals(ethRipemd160Addr)) {
      return ethRipemd160;
    }
    if (VMConfig.allowUvmCompatibleEvm() && address.equals(blake2FAddr)) {
      return blake2F;
    }
  ...
  public static class ValidateMultiSign extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] rawData) {
      ...
      byte[] address = words[0].toUnxAddress();
  ...
  public static class RewardBalance extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      long rewardBalance = VoteRewardUtil.queryReward(
          TransactionTrace.convertToUnxAddress(getCallerAddress()), getDeposit());
  ...
  public static class IsSrCandidate extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      byte[] address = new DataWord(data).toUnxAddress();
  ...
  public static class VoteCount extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      if (accountCapsule != null && !accountCapsule.getVotesList().isEmpty()) {
        ByteString witness = ByteString.copyFrom(words[1].toUnxAddress());
  ...
  public static class TotalVoteCount extends PrecompiledContract {
    ...
    public Pair<Boolean, byte[]> execute(byte[] data) {
      ...
      long power = accountCapsule != null
          ? accountCapsule.getUnwPower() / UNW_PRECISION : 0;
      return Pair.of(true, longTo32Bytes(power));
    }
  }
