...
public class UnfreezeBalanceProcessor {
  ...
  public long execute(UnfreezeBalanceParam param, Repository repo) {
    ...
    switch (param.getResourceType()) {
      case BANDWIDTH:
        repo.addTotalNetWeight(-unfreezeBalance / UNW_PRECISION);
        break;
      case ENERGY:
        repo.addTotalEnergyWeight(-unfreezeBalance / UNW_PRECISION);
        break;
      default:
        //this should never happen
        break;
    ...
    if (VMConfig.allowUvmVote() && !accountCapsule.getVotesList().isEmpty()) {
      long usedUnxPower = 0;
      for (Protocol.Vote vote : accountCapsule.getVotesList()) {
        usedUnxPower += vote.getVoteCount();
      }
      if (accountCapsule.getUnwPower() < usedUnxPower * UNW_PRECISION) {
