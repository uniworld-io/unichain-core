...
import static org.unx.common.utils.ByteUtil.stripLeadingZeroes;
...
import org.unx.common.utils.BIUtil;
import org.unx.common.utils.ByteUtil;
import org.unx.common.utils.FastByteComparisons;
...
import org.unx.core.vm.MessageCall;
import org.unx.core.vm.Op;
import org.unx.core.vm.OperationRegistry;
import org.unx.core.vm.PrecompiledContracts;
...
import org.unx.core.vm.nativecontract.FreezeBalanceProcessor;
import org.unx.core.vm.nativecontract.UnfreezeBalanceProcessor;
import org.unx.core.vm.nativecontract.VoteWitnessProcessor;
import org.unx.core.vm.nativecontract.WithdrawRewardProcessor;
import org.unx.core.vm.nativecontract.param.FreezeBalanceParam;
import org.unx.core.vm.nativecontract.param.UnfreezeBalanceParam;
...
import org.unx.core.vm.program.listener.CompositeProgramListener;
import org.unx.core.vm.program.listener.ProgramListenerAware;
import org.unx.core.vm.program.listener.ProgramStorageChangeListener;
...
import org.unx.core.vm.trace.ProgramTraceListener;
...
import org.unx.protos.contract.Common;
import org.unx.protos.contract.SmartContractOuterClass.SmartContract;
import org.unx.protos.contract.SmartContractOuterClass.SmartContract.Builder;
...
public class Program {
   ...
   * @param value the UNW value transferred in the internal transaction
   */
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] obtainer = obtainerAddress.toUnxAddress();

    if (VMConfig.allowUvmVote()) {
  ...
  private void transferDelegatedResourceToInheritor(byte[] ownerAddr, byte[] inheritorAddr, Repository repo) {
    ...
    repo.addTotalNetWeight(-frozenBalanceForBandwidthOfOwner / UNW_PRECISION);

    long frozenBalanceForEnergyOfOwner =
        ownerCapsule.getAccountResource().getFrozenBalanceForEnergy().getFrozenBalance();
    repo.addTotalEnergyWeight(-frozenBalanceForEnergyOfOwner / UNW_PRECISION);
...
//    return freezeCheck && voteCheck;
  ...
  private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress,
    ...
    if (VMConfig.allowUvmConstantinople()) {
      ...
      if (!contractAlreadyExists) {
        ...
        if (isCreate2) {
          builder.setUnxHash(ByteString.copyFrom(rootTransactionId));
        }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = msg.getCodeAddress().toUnxAddress();
    byte[] senderAddress = getContextAddress();
    ...
    if (!isTokenTransfer) {
      ...
      // transfer urc10 token validation
    ...
    if (byTestingSuite()) {
      ...
      if (!isTokenTransfer) {
        ...
        try {
          ...
          if (VMConfig.allowUvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), REFUND_ENERGY_FROM_MESSAGE_CALL);
            throw new TransferException("transfer urc10 failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
...
//  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(address.toUnxAddress());
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] addr = address.toUnxAddress();
    AccountCapsule account = getContractState().getAccount(addr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(addr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(addr, contract);
        }
      } else {
  ...
  public byte[] getContextAddress() {
    return invoke.getContractAddress().toUnxAddress();
  }
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState().getBalance(address.toUnxAddress());
    return new DataWord(balance);
  }

  public DataWord getRewardBalance(DataWord address) {
    long rewardBalance = VoteRewardUtil.queryReward(address.toUnxAddress(), getContractState());
    return new DataWord(rewardBalance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState().getContract(address.toUnxAddress());
    return contract != null ? DataWord.ONE() : DataWord.ZERO();
  }

  public DataWord isSRCandidate(DataWord address) {
    WitnessCapsule witnessCapsule = getContractState().getWitness(address.toUnxAddress());
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState().getTokenBalance(address.toUnxAddress(),
  ...
  public void createContract2(DataWord value, DataWord memStart, DataWord memSize, DataWord salt) {
    byte[] senderAddress;
    if (VMConfig.allowUvmCompatibleEvm() && getCallDeep() == MAX_DEPTH) {
      stackPushZero();
      return;
    }
    if (VMConfig.allowUvmIstanbul()) {
      senderAddress = getContextAddress();
    } else {
      senderAddress = getCallerAddress().toUnxAddress();
    }
    byte[] programCode = memoryChunk(memStart.intValue(), memSize.intValue());
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (msg.getOpCode() == Op.CALLCODE || msg.getOpCode() == Op.DELEGATECALL) {
      ...
      contextAddress = msg.getCodeAddress().toUnxAddress();
    }
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      if (msg.getOpCode() == Op.DELEGATECALL) {
        contract.setCallerAddress(getCallerAddress().toUnxAddress());
      } else {
  ...
  public DataWord getCallEnergy(DataWord requestedEnergy, DataWord availableEnergy) {
    if (VMConfig.allowUvmCompatibleEvm() && getContractVersion() == 1) {
  ...
  private void createAccountIfNotExist(Repository deposit, byte[] contextAddress) {
    if (VMConfig.allowUvmSolidity059()) {
      //after solidity059 proposal , allow contract transfer urc10 or UNW to non-exist address(would create one)
  ...
  public boolean freeze(DataWord receiverAddress, DataWord frozenBalance, DataWord resourceType) {
    ...
    byte[] owner = getContextAddress();
    byte[] receiver = receiverAddress.toUnxAddress();
    ...
    try {
      ...
      logger.error("UVM Freeze: validate failure. Reason: {}", e.getMessage());
    } catch (ArithmeticException e) {
      logger.error("UVM Freeze: frozenBalance out of long range.");
    }
    if (internalTx != null) {
      internalTx.reject();
    }
  ...
  public boolean unfreeze(DataWord receiverAddress, DataWord resourceType) {
    ...
    try {
      ...
      logger.error("UVM Unfreeze: validate failure. Reason: {}", e.getMessage());
    }
    if (internalTx != null) {
      internalTx.reject();
    }
  ...
  public long freezeExpireTime(DataWord targetAddress, DataWord resourceType) {
    byte[] owner = getContextAddress();
    byte[] target = targetAddress.toUnxAddress();
  ...
  public boolean voteWitness(int witnessArrayOffset, int witnessArrayLength,
    ...
    if (memoryLoad(witnessArrayOffset).intValueSafe() != witnessArrayLength ||
      ...
      logger.warn("UVM VoteWitness: memory array length do not match length parameter!");
      throw new BytecodeExecutionException(
          "UVM VoteWitness: memory array length do not match length parameter!");
    }

    if (witnessArrayLength != amountArrayLength) {
      logger.warn("UVM VoteWitness: witness array length {} does not match amount array length {}",
    ...
    try {
      ...
      for (int i = 0; i < witnessArrayLength; i++) {
        ...
        DataWord amount = new DataWord(Arrays.copyOfRange(amountArrayData,
            i * DataWord.WORD_SIZE, (i + 1) * DataWord.WORD_SIZE));
        param.addVote(witness.toUnxAddress(), amount.sValue().longValueExact());
      }
      if (internalTx != null) {
      ...
      logger.error("UVM VoteWitness: validate failure. Reason: {}", e.getMessage());
    } catch (ContractExeException e) {
      logger.error("UVM VoteWitness: execute failure. Reason: {}", e.getMessage());
    } catch (ArithmeticException e) {
      logger.error("UVM VoteWitness: int or long out of range. caused by: {}", e.getMessage());
    }
    if (internalTx != null) {
      internalTx.reject();
    }
  ...
  public long withdrawReward() {
    ...
    try {
      ...
      logger.error("UVM WithdrawReward: validate failure. Reason: {}", e.getMessage());
    } catch (ContractExeException e) {
      logger.error("UVM WithdrawReward: execute failure. Reason: {}", e.getMessage());
    }
    if (internalTx != null) {
      internalTx.reject();
    }
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(UnxException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(UnxException e) {
      return new PrecompiledContractException(e.getMessage());
    }
