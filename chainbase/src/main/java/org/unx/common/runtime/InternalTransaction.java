...
public class InternalTransaction {
  ...
  /* the amount of unx to transfer (calculated as ginza) */
  private long value;
   ...
   */
  public InternalTransaction(Transaction unx, UnxType unxType)
      throws ContractValidateException {
    this.transaction = unx;
    TransactionCapsule unxCap = new TransactionCapsule(unx);
    this.protoEncoded = unxCap.getData();
    this.nonce = 0;
    ...
    if (unxType == UnxType.UNX_CONTRACT_CREATION_TYPE) {
      ...
      this.transferToAddress = WalletUtil.generateContractAddress(unx);
      this.note = "create";
    ...
    this.hash = unxCap.getTransactionId().getBytes();
  }
  ...
  public InternalTransaction(byte[] parentHash, int deep, int index,
    ...
    // in this case, value also can represent a tokenValue when tokenId is not null, otherwise it is a unx callvalue.
    this.value = value;
    this.data = ArrayUtils.nullToEmpty(data);
    this.nonce = nonce;
    this.hash = getHash();
    // in a contract call contract case, only one value should be used. unx or a token. can't be both. We should avoid using
