...
import org.unx.protos.Protocol.Account.Builder;
import org.unx.protos.Protocol.Account.Frozen;
import org.unx.protos.Protocol.AccountType;
import org.unx.protos.Protocol.Key;
import org.unx.protos.Protocol.Permission;
import org.unx.protos.Protocol.Permission.PermissionType;
...
public class AccountCapsule implements ProtoCapsule<Account>, Comparable<AccountCapsule> {
  ...
  public long getAllUnwPower() {
    if (account.getOldUnwPower() == -1) {
      return getUnwPowerFrozenBalance();
    } else if (account.getOldUnwPower() == 0) {
      return getUnwPower() + getUnwPowerFrozenBalance();
    } else {
      return account.getOldUnwPower() + getUnwPowerFrozenBalance();
    }
  }
  ...
  public boolean oldUnwPowerIsNotInitialized() {
    return this.account.getOldUnwPower() == 0;
  }

  public boolean oldUnwPowerIsInvalid() {
    return this.account.getOldUnwPower() == -1;
  }

  public void initializeOldUnwPower() {
    long value = getUnwPower();
    if (value == 0) {
      value = -1;
    }
    setInstance(getInstance().toBuilder()
        .setOldUnwPower(value)
        .build());
  }

  public void invalidateOldUnwPower() {
    setInstance(getInstance().toBuilder()
        .setOldUnwPower(-1)
        .build());
  }
  ...
  public void setOldUnwPower(long value) {
    setInstance(getInstance().toBuilder()
        .setOldUnwPower(value)
        .build());
  }

  public void setFrozenForUnwPower(long frozenBalance, long expireTime) {
    Frozen newFrozen = Frozen.newBuilder()
        .setFrozenBalance(frozenBalance)
        .setExpireTime(expireTime)
        .build();
    ...
    setInstance(getInstance().toBuilder()
        .setUnwPower(newFrozen)
        .build());
  }

  public long getUnwPowerFrozenBalance() {
    return this.account.getUnwPower().getFrozenBalance();
  }
