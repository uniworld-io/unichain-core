...
import org.unx.core.capsule.utils.MerkleTree;
...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public void addTransaction(TransactionCapsule pendingUnw) {
    this.block = this.block.toBuilder().addTransactions(pendingUnw.getInstance()).build();
    getTransactions().add(pendingUnw);
  }
