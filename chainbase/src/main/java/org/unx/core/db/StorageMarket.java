...
public class StorageMarket {
  ...
  private long exchangeToSupply(boolean isUNW, long quant) {
    logger.info("isUNW: " + isUNW);
    long balance = isUNW ? dynamicPropertiesStore.getTotalStoragePool() :
...
//    }
  ...
  private long exchangeToSupply2(boolean isUNW, long quant) {
    logger.info("isUNW: " + isUNW);
    long balance = isUNW ? dynamicPropertiesStore.getTotalStoragePool() :
        dynamicPropertiesStore.getTotalStorageReserved();
    logger.info("balance: " + balance);
...
//    }
  ...
  public long exchange(long from, boolean isUNW) {
    long relay = exchangeToSupply(isUNW, from);
