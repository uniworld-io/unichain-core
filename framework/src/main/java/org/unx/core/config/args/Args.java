...
import static org.unx.core.config.Parameter.ChainConstant.BLOCK_PRODUCE_TIMEOUT_PERCENT;
import static org.unx.core.config.Parameter.ChainConstant.MAX_ACTIVE_WITNESS_NUM;
...
import org.unx.common.args.Account;
import org.unx.common.args.GenesisBlock;
import org.unx.common.args.Witness;
...
import org.unx.common.logsfilter.TriggerConfig;
...
import org.unx.common.parameter.RateLimiterInitialization;
...
import org.unx.common.utils.LocalWitnesses;
import org.unx.common.utils.PropUtil;
import org.unx.core.Constant;
import org.unx.core.Wallet;
import org.unx.core.config.Configuration;
import org.unx.core.config.Parameter.NetConstants;
import org.unx.core.config.Parameter.NodeConstant;
import org.unx.core.exception.CipherException;
import org.unx.core.store.AccountStore;
import org.unx.keystore.Credentials;
import org.unx.keystore.WalletUtils;
...
public class Args extends CommonParameter {
  ...
  public static void clearParam() {
    ...
    PARAMETER.allowUvmTransferUrc10 = 0;
    PARAMETER.allowUvmConstantinople = 0;
    PARAMETER.allowDelegateResource = 0;
    PARAMETER.allowSameTokenName = 0;
    PARAMETER.allowUvmSolidity059 = 0;
    ...
    PARAMETER.unxExpirationTimeInMilliseconds = 0;
    ...
    PARAMETER.allowShieldedURC20Transaction = 0;
    ...
    PARAMETER.allowUvmIstanbul = 0;
    PARAMETER.allowUvmFreeze = 0;
    PARAMETER.allowUvmVote = 0;
    PARAMETER.allowUvmLondon = 0;
    PARAMETER.allowUvmCompatibleEvm = 0;
  ...
  public static void setParam(final String[] args, final String confFileName) {
    ...
    PARAMETER.netMaxUnxPerSecond = config.hasPath(Constant.NODE_NET_MAX_UNX_PER_SECOND)
        ? config.getInt(Constant.NODE_NET_MAX_UNX_PER_SECOND)
        : NetConstants.NET_MAX_UNX_PER_SECOND;
    ...
    PARAMETER.allowUvmTransferUrc10 =
        config.hasPath(Constant.COMMITTEE_ALLOW_UVM_TRANSFER_URC10) ? config
            .getInt(Constant.COMMITTEE_ALLOW_UVM_TRANSFER_URC10) : 0;

    PARAMETER.allowUvmConstantinople =
        config.hasPath(Constant.COMMITTEE_ALLOW_UVM_CONSTANTINOPLE) ? config
            .getInt(Constant.COMMITTEE_ALLOW_UVM_CONSTANTINOPLE) : 0;

    PARAMETER.allowUvmSolidity059 =
        config.hasPath(Constant.COMMITTEE_ALLOW_UVM_SOLIDITY059) ? config
            .getInt(Constant.COMMITTEE_ALLOW_UVM_SOLIDITY059) : 0;
    ...
    PARAMETER.unxReferenceBlock = config.hasPath(Constant.UNX_REFERENCE_BLOCK)
        ? config.getString(Constant.UNX_REFERENCE_BLOCK) : "solid";

    PARAMETER.unxExpirationTimeInMilliseconds =
        config.hasPath(Constant.UNX_EXPIRATION_TIME_IN_MILLIS_SECONDS)
            && config.getLong(Constant.UNX_EXPIRATION_TIME_IN_MILLIS_SECONDS) > 0
            ? config.getLong(Constant.UNX_EXPIRATION_TIME_IN_MILLIS_SECONDS)
    ...
    PARAMETER.unxCacheEnable = config.hasPath(Constant.NODE_RPC_UNX_CACHE_ENABLE)
        && config.getBoolean(Constant.NODE_RPC_UNX_CACHE_ENABLE);
    ...
    PARAMETER.allowShieldedURC20Transaction =
        config.hasPath(Constant.COMMITTEE_ALLOW_SHIELDED_URC20_TRANSACTION) ? config
            .getInt(Constant.COMMITTEE_ALLOW_SHIELDED_URC20_TRANSACTION) : 0;
    ...
    PARAMETER.allowUvmIstanbul =
        config.hasPath(Constant.COMMITTEE_ALLOW_UVM_ISTANBUL) ? config
            .getInt(Constant.COMMITTEE_ALLOW_UVM_ISTANBUL) : 0;
    ...
    PARAMETER.allowUvmFreeze =
        config.hasPath(Constant.COMMITTEE_ALLOW_UVM_FREEZE) ? config
            .getInt(Constant.COMMITTEE_ALLOW_UVM_FREEZE) : 0;

    PARAMETER.allowUvmVote =
        config.hasPath(Constant.COMMITTEE_ALLOW_UVM_VOTE) ? config
            .getInt(Constant.COMMITTEE_ALLOW_UVM_VOTE) : 0;

    PARAMETER.allowUvmLondon =
        config.hasPath(Constant.COMMITTEE_ALLOW_UVM_LONDON) ? config
            .getInt(Constant.COMMITTEE_ALLOW_UVM_LONDON) : 0;

    PARAMETER.allowUvmCompatibleEvm =
        config.hasPath(Constant.COMMITTEE_ALLOW_UVM_COMPATIBLE_EVM) ? config
            .getInt(Constant.COMMITTEE_ALLOW_UVM_COMPATIBLE_EVM) : 0;
  ...
  public static void logConfig() {
    ...
    logger.info("Unw reference block: {}", parameter.getUnxReferenceBlock());
