...
import static org.unx.core.config.Parameter.ChainConstant.BLOCK_SIZE;
...
import org.unx.core.services.WitnessProductBlockService;
import org.unx.protos.Protocol.Inventory.InventoryType;

@Slf4j(topic = "net")
@Component
public class BlockMsgHandler implements UnxMsgHandler {
  ...
  public void processMessage(PeerConnection peer, UnxMessage msg) throws P2pException {
    ...
    if (peer.getSyncBlockRequested().containsKey(blockId)) {
      ...
      long interval = blockId.getNum() - unxNetDelegate.getHeadBlockId().getNum();
  ...
  private void processBlock(PeerConnection peer, BlockCapsule block) throws P2pException {
    ...
    if (!unxNetDelegate.containBlock(block.getParentBlockId())) {
      logger.warn("Get unlink block {} from {}, head is {}.", blockId.getString(),
              peer.getInetAddress(), unxNetDelegate.getHeadBlockId().getString());
    ...
    boolean flag = unxNetDelegate.validBlock(block);
    if (flag) {
    ...
    try {
      ...
      unxNetDelegate.getActivePeer().forEach(p -> {
