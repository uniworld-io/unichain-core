...
import static org.unx.common.utils.Commons.decodeFromBase58Check;
...
public class Util {
  ...
  public static byte[] generateContractAddress(Transaction unx, byte[] ownerAddress) {
    ...
    byte[] txRawDataHash = Sha256Hash
        .of(CommonParameter.getInstance().isECKeyCryptoEngine(), unx.getRawData().toByteArray())
        .getBytes();
  ...
  public static List<Log> convertLogAddressToUnxAddress(TransactionInfo transactionInfo) {
    ...
    for (Log log : transactionInfo.getLogList()) {
      ...
      if (oldAddress.length == 0 || oldAddress.length > 20) {
        ...
        logBuilder
            .setAddress(ByteString.copyFrom(TransactionTrace.convertToUnxAddress(newAddress)));
      }
