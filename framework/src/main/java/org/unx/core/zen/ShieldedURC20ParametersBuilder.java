...
import org.unx.common.zksnark.ZksnarkUtils;
import org.unx.core.Wallet;
import org.unx.core.capsule.ReceiveDescriptionCapsule;
import org.unx.core.capsule.SpendDescriptionCapsule;
...
@Slf4j
public class ShieldedURC20ParametersBuilder {
  ...
  @Getter
  private ShieldedURC20Parameters.Builder builder = ShieldedURC20Parameters.newBuilder();
  @Getter
  private long valueBalance = 0;
  @Getter
  @Setter
  private ShieldedURC20ParametersType shieldedURC20ParametersType;
  @Setter
  private byte[] shieldedURC20Address;
  @Setter
  ...
  public ShieldedURC20ParametersBuilder() {

  }

  public ShieldedURC20ParametersBuilder(String type) throws ZksnarkException {
    switch (type) {
      case "mint":
        shieldedURC20ParametersType = ShieldedURC20ParametersType.MINT;
        break;
      case "transfer":
        shieldedURC20ParametersType = ShieldedURC20ParametersType.TRANSFER;
        break;
      case "burn":
  ...
  public ShieldedURC20Parameters build(boolean withAsk) throws ZksnarkException {
    // Empty output script
    byte[] mergedBytes;
    ...
    ShieldContract.SpendDescription spendDescription;
    ShieldContract.ReceiveDescription receiveDescription;
    ShieldedURC20Parameters shieldedURC20Parameters;

    long ctx = JLibrustzcash.librustzcashSaplingProvingCtxInit();
    try {
      switch (shieldedURC20ParametersType) {
        case MINT:
          ...
          mergedBytes = ByteUtil.merge(shieldedURC20Address,
        ...
        case TRANSFER:
          ...
          mergedBytes = shieldedURC20Address;
    ...
    if (withAsk || shieldedURC20ParametersType == ShieldedURC20ParametersType.MINT) {
      shieldedURC20Parameters = builder.build();
      builder.setTriggerContractInput(
          getTriggerContractInput(shieldedURC20Parameters, null, value, true,
              transparentToAddress));
    }

    if (!withAsk && shieldedURC20ParametersType == ShieldedURC20ParametersType.BURN) {
      builder.setTriggerContractInput(Hex.toHexString(burnCiphertext));
    }
    return builder.build();
  }

  public String getTriggerContractInput(ShieldedURC20Parameters shieldedURC20Parameters,
      List<BytesMessage> spendAuthoritySignature,
      BigInteger value, boolean withAsk,
      byte[] transparentToAddress) {
    switch (shieldedURC20ParametersType) {
      case MINT:
        return mintParamsToHexString(shieldedURC20Parameters, value);
      case TRANSFER:
        return transferParamsToHexString(shieldedURC20Parameters, spendAuthoritySignature, withAsk);
      case BURN:
        return burnParamsToHexString(shieldedURC20Parameters, spendAuthoritySignature, value,
  ...
  private String mintParamsToHexString(GrpcAPI.ShieldedURC20Parameters mintParams,
      BigInteger value) {
  ...
  private String transferParamsToHexString(GrpcAPI.ShieldedURC20Parameters transferParams,
      List<BytesMessage> spendAuthoritySignature,
      boolean withAsk) {
  ...
  private String burnParamsToHexString(GrpcAPI.ShieldedURC20Parameters burnParams,
      List<BytesMessage> spendAuthoritySignature,
  ...
  public enum ShieldedURC20ParametersType {
    MINT,
    TRANSFER,
    BURN
  }
