...
public class NodeStatisticsTest {
  ...
  public void testMessage() {
    ...
    Assert.assertEquals(1, statistics.unxInSyncBlockChain.getTotalCount());
    ...
    Assert.assertEquals(1, statistics.unxOutBlockChainInventory.getTotalCount());

    InventoryMessage invMsgUnx =
        new InventoryMessage(new ArrayList<>(), Protocol.Inventory.InventoryType.UNX);
    String inventoryMessageStr = invMsgUnx.toString();
    Assert.assertNotNull(inventoryMessageStr);
    statistics.addTcpInMessage(invMsgUnx);
    statistics.addTcpOutMessage(invMsgUnx);
    ...
    Assert.assertEquals(1, statistics.unxInBlockInventory.getTotalCount());

    FetchInvDataMessage fetchInvDataUnx =
        new FetchInvDataMessage(new ArrayList<>(), Protocol.Inventory.InventoryType.UNX);
    statistics.addTcpInMessage(fetchInvDataUnx);
    statistics.addTcpOutMessage(fetchInvDataUnx);
    ...
    Assert.assertEquals(1, statistics.unxInUnxFetchInvData.getTotalCount());
    ...
    Assert.assertEquals(1, statistics.unxInUnxs.getTotalCount());
    ...
    long inBlockCount = statistics.unxInBlock.getTotalCount();
