...
public class Create2Test extends VMTestBase {
  ...
  */
  ...
  public void testCreate2()
    ...
    manager.getDynamicPropertiesStore().saveAllowUvmIstanbul(0);
    ...
    byte[] actualContract = convertToUnxAddress(Arrays.copyOfRange(returnValue,
        12, 32));
...
contract A {
  ...
  public void create2AddressTest() throws ContractExeException, ReceiptCheckErrException,
    ...
    manager.getDynamicPropertiesStore().saveAllowUvmTransferUrc10(1);
    manager.getDynamicPropertiesStore().saveAllowUvmConstantinople(1);
    manager.getDynamicPropertiesStore().saveAllowUvmSolidity059(1);
    manager.getDynamicPropertiesStore().saveAllowUvmIstanbul(1);
    String contractName = "Factory_0";
    byte[] address = Hex.decode(OWNER_ADDRESS);
    String abi = "[]";
    ...
    Transaction unx = UvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] factoryAddress = WalletUtil.generateContractAddress(unx);
    runtime = UvmTestUtils.processTransactionAndReturnRuntime(unx, rootDeposit, null);
    Assert.assertNull(runtime.getRuntimeError());
    ...
    // same input
    result = UvmTestUtils.triggerContractAndReturnUvmTestResult(Hex.decode(OWNER_ADDRESS),
              factoryAddress, Hex.decode(hexInput), 0, fee, manager, null);
    ...
    result = UvmTestUtils
        .triggerContractAndReturnUvmTestResult(Hex.decode(ownerAddress2),
            factoryAddress, Hex.decode(hexInput), 0, fee, manager, null);
