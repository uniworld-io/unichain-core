...
public class RepositoryTest {
  ...
  public void loopCallTest()
    ...
    Transaction aUnw = UvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null, engeryLiimt);
    Runtime runtime = UvmTestUtils.processTransactionAndReturnRuntime(aUnw,
    ...
    Transaction bUnw = UvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null, engeryLiimt);
    runtime = UvmTestUtils.processTransactionAndReturnRuntime(bUnw,
        RepositoryImpl.createRoot(StoreFactory.getInstance()), null);
    Assert.assertNull(runtime.getRuntimeError());

    byte[] aAddress = WalletUtil.generateContractAddress(aUnw);
    byte[] bAddress = WalletUtil.generateContractAddress(bUnw);
    ...
    // check result
    // expected: n1 = 1; n2 = 0
    byte[] checkN1Data = UvmTestUtils.parseAbi("n1()", null);
    byte[] checkN2Data = UvmTestUtils.parseAbi("n2()", null);

    UVMTestResult checkN1 = UvmTestUtils
        .triggerContractAndReturnUvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN1Data, 0, fee, manager, null);
    UVMTestResult checkN2 = UvmTestUtils
        .triggerContractAndReturnUvmTestResult(Hex.decode(OWNER_ADDRESS),
    ...
    triggerData = UvmTestUtils.parseAbi("callBcallA(address,uint256,uint256)", params2);
    result = UvmTestUtils
        .triggerContractAndReturnUvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, triggerData, 0, fee, manager, null);
    Assert.assertNull(result.getRuntime().getRuntimeError());
    checkN1 = UvmTestUtils
        .triggerContractAndReturnUvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN1Data, 0, fee, manager, null);
    checkN2 = UvmTestUtils
        .triggerContractAndReturnUvmTestResult(Hex.decode(OWNER_ADDRESS),
            aAddress, checkN2Data, 0, fee, manager, null);
    System.out.println(Hex.toHexString(checkN1.getRuntime().getResult().getHReturn()));
    System.out.println(Hex.toHexString(checkN2.getRuntime().getResult().getHReturn()));
  ...
  public void loopCallTestOldVersion()
    ...
    Runtime runtime = UvmTestUtils.processTransactionAndReturnRuntime(aUnw, rootRepository, null);
    Assert.assertNull(runtime.getRuntimeError());
    ...
    Transaction bUnw = UvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    rootRepository = RepositoryImpl.createRoot(StoreFactory.getInstance());
    runtime = UvmTestUtils.processTransactionAndReturnRuntime(bUnw, rootRepository, null);
    Assert.assertNull(runtime.getRuntimeError());
