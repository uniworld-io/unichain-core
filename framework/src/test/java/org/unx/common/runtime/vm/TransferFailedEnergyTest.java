...
public class TransferFailedEnergyTest extends VMTestBase {
  ...
  contract EnergyOfTransferFailedTest {
      ...
      // InsufficientBalance
      function testTransferUnwInsufficientBalance() payable public{
          msg.sender.transfer(10);
      }

      function testSendUnwInsufficientBalance() payable public{
          msg.sender.send(10);
      }
      ...
      function testCallUnwInsufficientBalance(address payable caller) public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }

      function testCreateUnxInsufficientBalance() payable public {
          (new Caller).value(10)();
      }

      // NonexistentTarget
      function testTransferUnwNonexistentTarget(address payable nonexistentTarget) payable public {
      ...
      function testCallUnwNonexistentTarget(address payable nonexistentTarget) payable public {
          require(address(this).balance >= 10);
      ...
      // target is self
      function testTransferUnwSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.transfer(10);
      }

      function testSendUnwSelf() payable public{
          require(address(this).balance >= 10);
          address payable self = address(uint160(address(this)));
          self.send(10);
      }
  ...
  contract EnergyOfTransferFailedTest {
      ...
      function testCallUnwInsufficientBalance(address caller) payable public {
          caller.call.value(10)(abi.encodeWithSignature("test()"));
      }
      ...
      // NonexistentTarget
      function testTransferUnwNonexistentTarget(address nonexistentTarget) payable public {
          require(address(this).balance >= 10);
          nonexistentTarget.transfer(10);
      }
      ...
      function testCallUnwNonexistentTarget(address nonexistentTarget) public {
          require(address(this).balance >= 10);
          nonexistentTarget.call.value(10)(abi.encodeWithSignature("test()"));
      }
  ...
  */
  ...
  TestCase[] testCasesAfterAllowUvmConstantinop = {
      new TestCase("testTransferUnwSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSendUnwSelf()", Collections.emptyList(), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testSuicideNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testTransferUnwNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
      new TestCase("testCallUnwNonexistentTarget(address)",
          Collections.singletonList(nonExistAddress), false,
          contractResult.TRANSFER_FAILED),
  };
  TestCase[] testCasesBeforeAllowUvmConstantinop = {
      new TestCase("testTransferUnwSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSendUnwSelf()", Collections.emptyList(),
          true, contractResult.UNKNOWN),
      new TestCase("testSuicideNonexistentTarget(address)",
  ...
  TestCase[] testCasesInsufficientBalance = {
      new TestCase("testTransferUnwInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
      new TestCase("testSendUnwInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.SUCCESS),
      new TestCase("testCreateUnxInsufficientBalance()", Collections.emptyList(),
          false,
          contractResult.REVERT),
  ...
  @Test
  public void testTransferFailedAfterAllowUvmConstantinopl()
      throws ContractExeException, ReceiptCheckErrException,
      VMIllegalException, ContractValidateException {
    ConfigLoader.disable = false;
    manager.getDynamicPropertiesStore().saveAllowUvmTransferUrc10(1);
    manager.getDynamicPropertiesStore().saveAllowUvmConstantinople(1);
    manager.getDynamicPropertiesStore().saveAllowUvmSolidity059(0);
    ...
    Transaction unx = UvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null);
    byte[] addressWithSufficientBalance = WalletUtil.generateContractAddress(unx);
    runtime = UvmTestUtils.processTransactionAndReturnRuntime(unx, rootRepository, null);
    Assert.assertNull(runtime.getRuntimeError());

    for (TestCase testCase : testCasesAfterAllowUvmConstantinop) {
      checkResult(testCase, addressWithSufficientBalance);
    }

    unx = UvmTestUtils.generateDeploySmartContractAndGetTransaction(
        contractName, address, ABI, code, 0, fee, consumeUserResourcePercent,
        null);
    byte[] addressWithoutBalance = WalletUtil.generateContractAddress(unx);
    runtime = UvmTestUtils.processTransactionAndReturnRuntime(unx, rootRepository, null);
    Assert.assertNull(runtime.getRuntimeError());
  ...
  @Test
  public void testTransferFailedBeforeAllowUvmConstantinopl()
      throws ContractExeException, ReceiptCheckErrException,
      VMIllegalException, ContractValidateException {
    ConfigLoader.disable = false;
    manager.getDynamicPropertiesStore().saveAllowUvmTransferUrc10(1);
    manager.getDynamicPropertiesStore().saveAllowUvmConstantinople(0);
    manager.getDynamicPropertiesStore().saveAllowUvmSolidity059(0);
    ...
    for (TestCase testCase : testCasesBeforeAllowUvmConstantinop) {
      checkResult(testCase, addressWithSufficientBalance);
    }
