...
import static org.unx.protos.Protocol.Transaction.Result.contractResult;
...
public class VoteTest {
  ...
  private static final long unw_precision = 1_000_000L;
  ...
  public void init() throws Exception {
    ...
    manager.getDynamicPropertiesStore().saveAllowUvmVote(1);
  }
  ...
  private byte[] deployContract(String contractName, String abi, String code) throws Exception {
    Protocol.Transaction unx = UvmTestUtils.generateDeploySmartContractAndGetTransaction(
        ...
        null, 1_000_000L);
    unx = unx.toBuilder().setRawData(
        unx.getRawData().toBuilder().setTimestamp(System.currentTimeMillis()).build()).build();
  ...
  private UVMTestResult triggerContract(byte[] contractAddr,
  ...
  public void testRewardAlgorithmNo1() throws Exception {
    ...
    // cycle-1
    {
      // freeze balance to get unx power
  ...
  private void voteWitness(byte[] contract,
                           ...
                           List<Long> unwPowerList) throws Exception {
    triggerContract(contract, SUCCESS, getEqualConsumer(1),
        voteMethod, witnessList, unwPowerList);
  }
