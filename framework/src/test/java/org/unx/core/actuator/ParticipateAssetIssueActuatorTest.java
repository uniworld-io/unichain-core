...
public class ParticipateAssetIssueActuatorTest {
  ...
  private static final int UNX_NUM = 2;
  ...
  private static long AMOUNT = TOTAL_SUPPLY - (1000L) / UNX_NUM * NUM;
  ...
  public void sameTokenNameOpenRightAssetIssue() {
    ...
    try {
      ...
      Assert.assertEquals(owner.getAssetV2MapForTest()
                      .get(String.valueOf(id)).longValue(), (1000L) / UNX_NUM * NUM);
      Assert.assertEquals(
          toAccount.getAssetV2MapForTest().get(String.valueOf(id)).longValue(),
          TOTAL_SUPPLY - (1000L) / UNX_NUM * NUM);
    } catch (ContractValidateException e) {
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
  ...
  /**
   * SameTokenName close, not enough unx
   */
  @Test
  public void sameTokenNameCloseNotEnoughUnwTest() {
    initAssetIssue(chainBaseManager.getDynamicPropertiesStore()
            .getLatestBlockHeaderTimestamp() - 1000,
        chainBaseManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() + 1000);
    // First, reduce the owner unx balance. Else can't complete this test case.
    AccountCapsule owner = chainBaseManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
  ...
  /**
   * SameTokenName open, not enough unx
   */
  @Test
  public void sameTokenNameOpenNotEnoughUnwTest() {
    chainBaseManager.getDynamicPropertiesStore().saveAllowSameTokenName(1);
    initAssetIssue(chainBaseManager.getDynamicPropertiesStore()
            .getLatestBlockHeaderTimestamp() - 1000,
        chainBaseManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() + 1000);
  ...
  public void sameTokenNameCloseMultiplyOverflowTest() {
    initAssetIssue(chainBaseManager.getDynamicPropertiesStore()
            .getLatestBlockHeaderTimestamp() - 1000,
        chainBaseManager.getDynamicPropertiesStore().getLatestBlockHeaderTimestamp() + 1000);
    // First, increase the owner unx balance. Else can't complete this test case.
    AccountCapsule owner = chainBaseManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
  ...
  /**
   * SameTokenName close, exchangeAmount <= 0 unx, throw exception
   */
  @Test
  ...
  /**
   * SameTokenName open, exchangeAmount <= 0 unx, throw exception
   */
  @Test
