...
public class UnfreezeBalanceActuatorTest {
  ...
  private Any getContractForUnxPower(String ownerAddress) {
    return Any.pack(UnfreezeBalanceContract.newBuilder()
        .setOwnerAddress(ByteString.copyFrom(ByteArray.fromHexString(ownerAddress)))
        .setResource(ResourceCode.UNX_POWER).build());
  }
  ...
  public void testUnfreezeBalanceForBandwidth() {
    ...
    Assert.assertEquals(accountCapsule.getUnwPower(), frozenBalance);
    ...
    try {
      ...
      Assert.assertEquals(owner.getUnwPower(), 0L);
  ...
  public void testUnfreezeDelegatedBalanceForBandwidth() {
    ...
    try {
      ...
      Assert.assertEquals(0L, ownerResult.getUnwPower());
  ...
  public void testUnfreezeDelegatedBalanceForBandwidthWithDeletedReceiver() {
    ...
    dbManager.getDynamicPropertiesStore().saveAllowUvmConstantinople(0);
  ...
  public void testUnfreezeDelegatedBalanceForBandwidthWithRecreatedReceiver() {
    ...
    dbManager.getDynamicPropertiesStore().saveAllowUvmSolidity059(0);
    try {
      actuator.validate();
      actuator.execute(ret);
      Assert.fail();
  ...
  @Test
  public void testUnfreezeBalanceForEnergyWithOldUnxPowerAfterNewResourceModel() {
    long now = System.currentTimeMillis();
    dbManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(now);
    ...
    AccountCapsule accountCapsule = dbManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
    accountCapsule.setFrozenForEnergy(frozenBalance, now);
    accountCapsule.setOldUnwPower(frozenBalance);
    ...
    try {
      ...
      Assert.assertEquals(owner.getInstance().getOldUnwPower(), -1L);
    } catch (ContractValidateException e) {
      Assert.assertFalse(e instanceof ContractValidateException);
    } catch (ContractExeException e) {
      Assert.assertFalse(e instanceof ContractExeException);
  ...
  @Test
  public void testUnfreezeBalanceForEnergyWithoutOldUnxPowerAfterNewResourceModel() {
    long now = System.currentTimeMillis();
    dbManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(now);
    dbManager.getDynamicPropertiesStore().saveAllowNewResourceModel(1L);
    ...
    AccountCapsule accountCapsule = dbManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
    accountCapsule.setFrozenForEnergy(frozenBalance, now);
    accountCapsule.setOldUnwPower(-1L);
    accountCapsule.addVotes(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)), 100L);
  ...
  @Test
  public void testUnfreezeBalanceForUnxPowerWithOldUnxPowerAfterNewResourceModel() {
    long now = System.currentTimeMillis();
    dbManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(now);
    dbManager.getDynamicPropertiesStore().saveAllowNewResourceModel(1L);
    ...
    AccountCapsule accountCapsule = dbManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
    accountCapsule.setFrozenForEnergy(frozenBalance, now);
    accountCapsule.setFrozenForUnwPower(frozenBalance, now);
    accountCapsule.addVotes(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)), 100L);

    dbManager.getAccountStore().put(accountCapsule.createDbKey(), accountCapsule);
    UnfreezeBalanceActuator actuator = new UnfreezeBalanceActuator();
    actuator.setChainBaseManager(dbManager.getChainBaseManager())
        .setAny(getContractForUnxPower(OWNER_ADDRESS));
    TransactionResultCapsule ret = new TransactionResultCapsule();
  ...
  @Test
  public void testUnfreezeBalanceForUnxPowerWithOldUnxPowerAfterNewResourceModelError() {
    long now = System.currentTimeMillis();
    dbManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(now);
    dbManager.getDynamicPropertiesStore().saveAllowNewResourceModel(1L);
    ...
    AccountCapsule accountCapsule = dbManager.getAccountStore()
        .get(ByteArray.fromHexString(OWNER_ADDRESS));
    accountCapsule.setFrozenForEnergy(frozenBalance, now);
    accountCapsule.setFrozenForUnwPower(frozenBalance, now + 100000000L);
    accountCapsule.addVotes(ByteString.copyFrom(ByteArray.fromHexString(OWNER_ADDRESS)), 100L);
