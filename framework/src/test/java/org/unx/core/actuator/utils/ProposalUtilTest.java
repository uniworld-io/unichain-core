package org.unx.core.actuator.utils;
...
public class ProposalUtilTest {
  ...
  public void validProposalTypeCheck() throws ContractValidateException {
    ...
    Assert.assertEquals(ProposalType.ALLOW_UVM_SOLIDITY_059, ProposalType.getEnumOrNull(32));
    ...
    code = 32;
    Assert.assertEquals(ProposalType.ALLOW_UVM_SOLIDITY_059, ProposalType.getEnum(code));
  ...
  public void validateCheck() {
    ...
    try {
      actuatorUtil.validator(dynamicPropertiesStore, forkUtils,
          ProposalType.ALLOW_UVM_TRANSFER_URC10.getCode(), 2);
      Assert.assertTrue(false);
    } catch (ContractValidateException e) {
      Assert.assertEquals(
          "This value[ALLOW_UVM_TRANSFER_URC10] is only allowed to be 1", e.getMessage());
    }

    dynamicPropertiesStore.saveAllowSameTokenName(0);
    try {
      actuatorUtil.validator(dynamicPropertiesStore, forkUtils,
          ProposalType.ALLOW_UVM_TRANSFER_URC10.getCode(), 1);
      Assert.assertTrue(false);
    } catch (ContractValidateException e) {
      Assert.assertEquals("[ALLOW_SAME_TOKEN_NAME] proposal must be approved "
          + "before [ALLOW_UVM_TRANSFER_URC10] can be proposed", e.getMessage());
    }
    ...
    try {
      ProposalUtil.validator(dynamicPropertiesStore, forkUtils,
          ProposalType.ALLOW_SHIELDED_URC20_TRANSACTION
              .getCode(), 2);
      Assert.fail();
    } catch (ContractValidateException e) {
      Assert.assertEquals("This value[ALLOW_SHIELDED_URC20_TRANSACTION] is only allowed"
