...
public class TransactionCapsuleTest {
  ...
  public void addSign() {
    ...
    Transaction.Builder unxBuilder = Transaction.newBuilder();
    ...
    unxBuilder.setRawData(rawBuilder);
    AccountStore accountStore = dbManager.getAccountStore();
    TransactionCapsule transactionCapsule = new TransactionCapsule(unxBuilder.build());
    //Accout not exist
    try {
  ...
  public void validateSignature0() {
    ...
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    unxBuilder.clearSignature().addSignature(test);
    try {
      TransactionCapsule.validateSignature(unxBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    new Random().nextBytes(rand);
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    unxBuilder.clearSignature().addSignature(ByteString.copyFrom(rand));
    try {
      TransactionCapsule.validateSignature(unxBuilder.build(), hash,
          accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertEquals(e.getMessage(), "Header byte out of range: 35");
    ...
    unxBuilder.clearSignature().addAllSignature(sign21_11);
    try {
      TransactionCapsule.validateSignature(unxBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
    ...
    unxBuilder.clearSignature().addAllSignature(sign21_11_22_23);
    try {
      TransactionCapsule
          .validateSignature(unxBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    } catch (SignatureException e) {
      Assert.assertFalse(true);
    } catch (PermissionException e) {
    ...
    unxBuilder.clearSignature().addAllSignature(sign21_22_21);
    try {
      TransactionCapsule
          .validateSignature(unxBuilder.build(), hash, accountStore);
      Assert.assertFalse(true);
    ...
    unxBuilder.clearSignature().addAllSignature(sign21);
    try {
    ...
    unxBuilder.clearSignature().addAllSignature(sign21_22);
    try {
      boolean result = TransactionCapsule
          .validateSignature(unxBuilder.build(), hash, accountStore);
    ...
    unxBuilder.clearSignature().addAllSignature(sign21_22_23);
    try {
      boolean result = TransactionCapsule
          .validateSignature(unxBuilder.build(), hash, accountStore);
      Assert.assertTrue(result);
  ...
  public void validateSignature1() {
    ...
    ArrayList<ByteString> sign = sign(prikeys, Sha256Hash.hash(rawBuilder.build().toByteArray()));
    unxBuilder.addAllSignature(sign);
    transactionCapsule = new TransactionCapsule(unxBuilder.build());

    // no contract
    prikeys.clear();
    prikeys.add(ByteArray.fromHexString(KEY_21));
    unxBuilder = Transaction.newBuilder();
    ...
    unxBuilder = transactionCapsule.getInstance().toBuilder();
    //SignatureFormatException
    ByteString test = ByteString.copyFromUtf8("test");
    unxBuilder.clearSignature();
    unxBuilder.addSignature(test);
    transactionCapsule = new TransactionCapsule(unxBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    rand[64] = 8;  // v = 8 < 27 v += 35 > 35
    unxBuilder.clearSignature();
    unxBuilder.addSignature(ByteString.copyFrom(rand));
    transactionCapsule = new TransactionCapsule(unxBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11 = sign(prikeys, hash);
    unxBuilder.clearSignature();
    unxBuilder.addAllSignature(sign21_11);
    transactionCapsule = new TransactionCapsule(unxBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_11_22_23 = sign(prikeys, hash);
    unxBuilder.clearSignature();
    unxBuilder.addAllSignature(sign21_11_22_23);
    transactionCapsule = new TransactionCapsule(unxBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22_21 = sign(prikeys, hash);
    unxBuilder.clearSignature();
    unxBuilder.addAllSignature(sign21_22_21);
    transactionCapsule = new TransactionCapsule(unxBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21 = sign(prikeys, hash);
    unxBuilder.clearSignature();
    unxBuilder.addAllSignature(sign21);
    transactionCapsule = new TransactionCapsule(unxBuilder.build());
    try {
      transactionCapsule.validateSignature(dbManager);
      Assert.assertFalse(true);
    ...
    ArrayList<ByteString> sign21_22 = sign(prikeys, hash);
    unxBuilder.clearSignature();
    unxBuilder.addAllSignature(sign21_22);
    transactionCapsule = new TransactionCapsule(unxBuilder.build());
    try {
    ...
    ArrayList<ByteString> sign21_22_23 = sign(prikeys, hash);
    unxBuilder.clearSignature();
    unxBuilder.addAllSignature(sign21_22_23);
    transactionCapsule = new TransactionCapsule(unxBuilder.build());
    try {
      boolean result = transactionCapsule.validateSignature(dbManager);
      Assert.assertTrue(result);
  ...
  @Test
  public void unxCapsuleClearTest() {
    Transaction tx = Transaction.newBuilder()
        .addRet(Result.newBuilder().setContractRet(contractResult.OUT_OF_TIME).build()).build();
    TransactionCapsule unxCap = new TransactionCapsule(tx);
    Result.contractResult contractResult = unxCap.getContractResult();
    unxCap.resetResult();
    Assert.assertEquals(unxCap.getInstance().getRetCount(), 0);
    unxCap.setResultCode(contractResult);
    Assert.assertEquals(unxCap.getInstance()
