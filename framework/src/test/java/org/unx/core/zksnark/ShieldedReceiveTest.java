...
import org.unx.common.zksnark.LibrustzcashParam.BindingSigParams;
import org.unx.common.zksnark.LibrustzcashParam.CheckOutputParams;
import org.unx.common.zksnark.LibrustzcashParam.CheckSpendParams;
import org.unx.common.zksnark.LibrustzcashParam.IvkToPkdParams;
import org.unx.common.zksnark.LibrustzcashParam.OutputProofParams;
...
import org.unx.core.zen.ZenTransactionBuilder.ReceiveDescriptionInfo;
import org.unx.core.zen.ZenTransactionBuilder.SpendDescriptionInfo;
...
import org.unx.core.zen.note.Note.NotePlaintextEncryptionResult;
...
public class ShieldedReceiveTest extends BlockGenerate {
  ...
  private static final String ASSET_NAME = "unw";
  ...
  private static byte[] randomUint256() {
    return org.unx.keystore.Wallet.generateRandomBytes(32);
  }

  private static byte[] randomUint640() {
    return org.unx.keystore.Wallet.generateRandomBytes(80);
  }

  private static byte[] randomUint1536() {
    return org.unx.keystore.Wallet.generateRandomBytes(192);
  }

  private static byte[] randomUint4640() {
    return org.unx.keystore.Wallet.generateRandomBytes(580);
  }
 ...
 */
  ...
  public void testDiversifierT() throws ZksnarkException {
    //byte[] data = org.unx.keystore.Wallet.generateRandomBytes(Constant.ZC_DIVERSIFIER_SIZE);
  ...
  public void testMemoTooLong() throws ContractValidateException, TooBigTransactionException,
    ...
    byte[] memo = org.unx.keystore.Wallet.generateRandomBytes(1024);
    ...
    for (org.unx.protos.Protocol.Transaction.Contract c : t.getRawData().getContractList()) {
  ...
  public void testMemoNotEnough() throws ContractValidateException, TooBigTransactionException,
    ...
    IncomingViewingKey incomingViewingKey = fullViewingKey.inViewingKey();
    PaymentAddress paymentAddress = incomingViewingKey.address(new DiversifierT()).get();
    byte[] memo = org.unx.keystore.Wallet.generateRandomBytes(128);
    builder.addOutput(expsk.getOvk(), paymentAddress,
        100 * 1000000L - wallet.getShieldedTransactionFee(), memo);
  ...
  public void pushSameSkAndScanAndSpend() throws Exception {
    ...
    byte[] memo = org.unx.keystore.Wallet.generateRandomBytes(512);
    ...
    byte[] unxId = transactionCap.getTransactionId().getBytes();
    boolean ok = dbManager.pushTransaction(transactionCap);
    Assert.assertTrue(ok);
    ...
    for (int i = 0; i < notes1.getNoteTxsCount(); i++) {
      ...
      outPointBuild.setHash(ByteString.copyFrom(unxId));
    ...
    for (int i = 0; i < 1; i++) {
      org.unx.api.GrpcAPI.Note grpcNote = notes1.getNoteTxs(i).getNote();
    ...
    byte[] memo3 = org.unx.keystore.Wallet.generateRandomBytes(512);
  ...
  public void decodePaymentAddressIgnoreCase() {
    String addressLower =
        "zunw1975m0wyg8f30cgf2l5fgndhzqzkzgkgnxge8cwx2wr7m3q7chsuwewh2e6u24yykum0hq8ue99u";
