...
public class HttpMethed {
  ...
  public static HttpResponse assetIssue(
    ...
    try {
      ...
      userBaseObj2.addProperty("unx_num", unxNum);
  ...
  public static HttpResponse freezeBalance(
    ...
    try {
      ...
      if (resourceCode == 2) {
        userBaseObj2.addProperty("resource", "UNX_POWER");
      }
  ...
  /** constructor. */
  public static Long getBurnUnw(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/wallet/getburnunw";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
    ...
    responseContent = HttpMethed.parseResponseContent(response);
    return responseContent.getLong("burnUnwAmount");
  }

  /** constructor. */
  public static Long getBurnUnwFromSolidity(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/walletsolidity/getburnunw";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
  ...
  /** constructor. */
  public static Long getBurnUnwFromPbft(String httpNode) {
    try {
      final String requestUrl = "http://" + httpNode + "/walletpbft/getburnunw";
      JsonObject blockObj = new JsonObject();
      response = createConnect(requestUrl, blockObj);
    } catch (Exception e) {
      e.printStackTrace();
  ...
  public static HttpResponse sendShieldCoin(
    ...
    try {
      ...
      if (shieldAddressInfo != null) {
        ...
        response =
            HttpMethed.getMerkleTreeVoucherInfo(httpNode, noteTx.getUnxId(), noteTx.getIndex(), 1);
        responseContent = HttpMethed.parseResponseContent(response);
  ...
  public static List<ShieldNoteInfo> scanNoteByOvkFromPbft(
    ...
    try {
      ...
      for (int i = 0; i < jsonArray.size(); i++) {
        ...
        noteTx.setUnxId(HttpMethed.parseStringContent(noteTxs).getString("txid"));
  ...
  public static HttpResponse sendShieldCoinWithoutAsk(
    ...
    try {
      ...
      if (shieldAddressInfo != null) {
        ...
        logger.info("noteTx.getUnxId():" + noteTx.getUnxId());
        HttpMethed.response =
            HttpMethed.getMerkleTreeVoucherInfoFromSolidity(
                httpSolidityNode, noteTx.getUnxId(), noteTx.getIndex(), 1);
        ...
        HttpMethed.response =
            HttpMethed.getMerkleTreeVoucherInfoFromPbft(
                httpPbftNode, noteTx.getUnxId(), noteTx.getIndex(), 1);
        HttpMethed.responseContent = HttpMethed.parseResponseContent(HttpMethed.response);
        HttpMethed.printJsonContent(responseContent);
