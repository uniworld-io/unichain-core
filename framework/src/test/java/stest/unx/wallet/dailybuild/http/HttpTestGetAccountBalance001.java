package stest.unx.wallet.dailybuild.http;
...
public class HttpTestGetAccountBalance001 {
  ...
  public void test01DeployContractForTest() {
    ...
    String code = Configuration.getByPath("testng.conf")
        .getString("code.code_ContractUrcToken001_transferTokenContract");
    String abi = Configuration.getByPath("testng.conf")
        .getString("abi.abi_ContractUrcToken001_transferTokenContract");
    txid = HttpMethed
   ...
   * constructor.
   */
  @Test(enabled = true, description = "Get burn unx by http")
  public void test03GetBurnUnw() {
    ...
    HttpMethed.waitToProduceOneBlock(httpnode);
    final Long beforeBurnUnwAmount = HttpMethed.getBurnUnw(httpnode);
    ECKey ecKey3 = new ECKey(Utils.getRandom());
    ...
    HttpMethed.waitToProduceOneBlockFromSolidity(httpnode, httpSolidityNode);
    Long afterBurnUnwAmount = HttpMethed.getBurnUnw(httpnode);
    logger.info(afterBurnUnwAmount + "  :   " + beforeBurnUnwAmount);
    Assert.assertTrue(afterBurnUnwAmount - beforeBurnUnwAmount == 100000L);

    Assert.assertEquals(afterBurnUnwAmount, HttpMethed.getBurnUnwFromSolidity(httpSolidityNode));
    Assert.assertEquals(afterBurnUnwAmount, HttpMethed.getBurnUnwFromPbft(httpPbftNode));
  }
