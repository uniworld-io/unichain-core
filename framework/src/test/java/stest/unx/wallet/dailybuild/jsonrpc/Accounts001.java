...
public class Accounts001 extends JsonRpcBase {
  ...
  @Test(enabled = true, description = "Json rpc api of eth_estimateGasSendUnx")
  public void test09JsonRpcApiTestForEthEstimateGasSendUnx() throws Exception {
    JsonObject param = new JsonObject();
    param.addProperty("from", ByteArray.toHexString(jsonRpcOwnerAddress));
  ...
  public void test21JsonRpcApiTestForEthGetTransactionReceipt() throws Exception {
    logger.info("urc20Txid:" + urc20Txid);
    JsonArray params = new JsonArray();
    Thread.sleep(6000);
    params.add(urc20Txid);
    ...
    response = HttpMethed.getBlockByNum(httpFullNode, blockNumForUrc20);
    responseContent = HttpMethed.parseResponseContent(response);
    int index = 0;
    for (int i = 0; i < responseContent.getJSONArray("transactions").size(); i++) {
      if (urc20Txid.equals(
    ...
    Assert.assertEquals(
        resultFromTransactionReceipt.getString("transactionHash"), "0x" + urc20Txid);
    Assert.assertEquals(
    ...
    response = HttpMethed.getTransactionInfoByBlocknum(httpFullNode, blockNumForUrc20);
    List<JSONObject> responseContent1 = HttpMethed.parseResponseContentArray(response);
