package stest.unx.wallet.dailybuild.originenergylimit;
...
public class ContractOriginEnergyLimit004 {
  ...
  public void testOriginEnergyLimit() {
    ...
    long devFreezeBalanceGinza = PublicMethed.getFreezeBalanceCount(dev001Address, dev001Key,
        devTargetEnergy, blockingStubFull);

    long devNeedBalance = devTargetBalance + devFreezeBalanceGinza;
    ...
    PublicMethed.waitProduceNextBlock(blockingStubFull);
    // get energy
    Assert.assertTrue(PublicMethed.freezeBalanceGetEnergy(dev001Address, devFreezeBalanceGinza,
    ...
    // count dev energy, balance
    devFreezeBalanceGinza = PublicMethed.getFreezeBalanceCount(dev001Address, dev001Key,
        devTriggerTargetEnergy, blockingStubFull);

    devNeedBalance = devTriggerTargetBalance + devFreezeBalanceGinza;
    logger.info("dev need  balance:" + devNeedBalance);

    // count user energy, balance
    long userFreezeBalanceGinza = PublicMethed.getFreezeBalanceCount(user001Address, user001Key,
        userTargetEnergy, blockingStubFull);

    long userNeedBalance = userTargetBalance + userFreezeBalanceGinza;
    ...
    Assert.assertTrue(PublicMethed.freezeBalanceGetEnergy(dev001Address, devFreezeBalanceGinza,
        0, 1, dev001Key, blockingStubFull));
    Assert.assertTrue(PublicMethed.freezeBalanceGetEnergy(user001Address, userFreezeBalanceGinza,
