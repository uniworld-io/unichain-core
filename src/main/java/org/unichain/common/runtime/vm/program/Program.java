...
import org.unichain.common.runtime.vm.program.listener.CompositeProgramListener;
import org.unichain.common.runtime.vm.program.listener.ProgramListenerAware;
import org.unichain.common.runtime.vm.program.listener.ProgramStorageChangeListener;
import org.unichain.common.runtime.vm.trace.ProgramTrace;
import org.unichain.common.runtime.vm.trace.ProgramTraceListener;
...
import org.unichain.core.actuator.TransferAssetActuator;
...
public class Program {
  ...
  public void suicide(DataWord obtainerAddress) {
    ...
    byte[] owner = convertToUnichainAddress(getContractAddress().getLast20Bytes());
    byte[] obtainer = convertToUnichainAddress(obtainerAddress.getLast20Bytes());
    ...
    if (FastByteComparisons.compareTo(owner, 0, 20, obtainer, 0, 20) == 0) {
      ...
      byte[] burnAccountAddress = getContractState().getBurnAccountAddress();
      if (VMConfig.allowTvmTransferUnc()) {
        getContractState().addBalance(burnAccountAddress, balance);
        transferAllToken(getContractState(), owner, burnAccountAddress);
      }
    } else {
    ...
    if (refundEnergy > 0) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("The remaining energy is refunded, account: [{}], energy: [{}] ",
            Hex.toHexString(convertToUnichainAddress(getContractAddress().getLast20Bytes())),
            refundEnergy);
      }
    }
  }
  ...
  public void callToAddress(MessageCall msg) {
    ...
    byte[] codeAddress = convertToUnichainAddress(msg.getCodeAddress().getLast20Bytes());
    byte[] senderAddress = convertToUnichainAddress(getContractAddress().getLast20Bytes());
    ...
    if (byTestingSuite()) {
      ...
      if (!isTokenTransfer) {
        ...
        try {
          ...
          if (VMConfig.allowTvmConstantinople()) {
            refundEnergy(msg.getEnergy().longValue(), "refund energy from message call");
            throw new TransferException("transfer unc failed: %s", e.getMessage());
          }
          throw new BytecodeExecutionException(VALIDATE_FOR_SMART_CONTRACT_FAILURE, e.getMessage());
        }
  ...
  public void spendEnergy(long energyValue, String opName) {
    if (getEnergylimitLeftLong() < energyValue) {
      throw new OutOfEnergyException("Not enough energy for '%s' operation executing: curInvokeEnergyLimit[%d]," + " curOpEnergy[%d], usedEnergy[%d]",
  ...
  public void storageSave(DataWord word1, DataWord word2) {
    ...
    getContractState()
        .putStorageValue(convertToUnichainAddress(getContractAddress().getLast20Bytes()), keyWord,
            valWord);
  }
  ...
  public byte[] getCodeAt(DataWord address) {
    byte[] code = invoke.getDeposit().getCode(convertToUnichainAddress(address.getLast20Bytes()));
    return nullToEmpty(code);
  }

  public byte[] getCodeHashAt(DataWord address) {
    byte[] unichainAddr = convertToUnichainAddress(address.getLast20Bytes());
    AccountCapsule account = getContractState().getAccount(unichainAddr);
    if (account != null) {
      ContractCapsule contract = getContractState().getContract(unichainAddr);
      byte[] codeHash;
      if (contract != null) {
        ...
        if (ByteUtil.isNullOrZeroArray(codeHash)) {
          ...
          getContractState().updateContract(unichainAddr, contract);
        }
      } else {
  ...
  public DataWord getBalance(DataWord address) {
    long balance = getContractState().getBalance(convertToUnichainAddress(address.getLast20Bytes()));
    return new DataWord(balance);
  }

  public DataWord isContract(DataWord address) {
    ContractCapsule contract = getContractState()
        .getContract(convertToUnichainAddress(address.getLast20Bytes()));
  ...
  public DataWord storageLoad(DataWord key) {
    DataWord ret = getContractState()
        .getStorageValue(convertToUnichainAddress(getContractAddress().getLast20Bytes()), key.clone());
  ...
  public DataWord getTokenBalance(DataWord address, DataWord tokenId) {
    ...
    long ret = getContractState().getTokenBalance(convertToUnichainAddress(address.getLast20Bytes()),
  ...
  public void callToPrecompiledAddress(MessageCall msg,
    ...
    if (requiredEnergy > msg.getEnergy().longValue()) {
      ...
      contract.setCallerAddress(convertToUnichainAddress(msg.getType().callIsDelegate()
  ...
  public static class Exception {
    ...
    public static PrecompiledContractException contractValidateException(UnichainException e) {
      return new PrecompiledContractException(e.getMessage());
    }

    public static PrecompiledContractException contractExecuteException(UnichainException e) {
      return new PrecompiledContractException(e.getMessage());
    }
  ...
  private void createAccountIfNotExist(Deposit deposit, byte[] contextAddress) {
    if (VMConfig.allowTvmSolidity059()) {
      //after solidity059 proposal , allow contract transfer unc or unx to non-exist address(would create one)
