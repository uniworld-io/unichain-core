...
public class DepositImpl implements Deposit {
  ...
  public AccountCapsule createNormalAccount(byte[] address) {
    ...
    AccountCapsule account = new AccountCapsule(ByteString.copyFrom(address), AccountType.Normal, dbManager.getHeadBlockTimeStamp(), withDefaultPermission, dbManager);
  ...
  @Override
  public byte[] getBurnAccountAddress() {
  ...
  public synchronized Storage getStorage(byte[] address) {
    ...
    ContractCapsule contract = getContract(address);
    if (contract != null && !ByteUtil.isNullOrZeroArray(contract.getUnxHash())) {
      storage.generateAddrHash(contract.getUnxHash());
    }
    return storage;
  }
  ...
  public synchronized void putStorageValue(byte[] address, DataWord key, DataWord value) {
    address = convertToUnichainAddress(address);
...
//    accountCapsule.addAssetAmount(tokenIdWithoutLeadingZero, value);
  ...
  @Override
  public TransactionCapsule getTransaction(byte[] unxHash) {
    Key key = Key.create(unxHash);
    ...
    if (parent != null) {
      transactionCapsule = parent.getTransaction(unxHash);
    } else {
      try {
        transactionCapsule = getTransactionStore().get(unxHash);
