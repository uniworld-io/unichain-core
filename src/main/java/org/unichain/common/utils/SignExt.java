...
public class SignExt {
    ...
    public static byte[] recoverAddress(byte[] signatureBytes, byte[] message) throws SignatureException {
        var signature = ByteArray.toHexString(signatureBytes);
        String r = signature.substring(0, 66);
        String s = "0x" + signature.substring(66, 130);
        String v = "0x" + signature.substring(130, 132);

        String pubKey = signedMessageToKey(message,
                new SignatureData(Numeric.hexStringToByteArray(v)[0],
                        Numeric.hexStringToByteArray(r),
                        Numeric.hexStringToByteArray(s))).toString(16);

        return ByteArray.fromHexString(Keys.getAddress(pubKey));
    }
