...
@Component
@Slf4j(topic = "API")
public class NftAddMinterServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      var build = Contract.AddNftMinterContract.newBuilder();
      JsonFormat.merge(contract, build, visible);
      var tokenCtx = build.build();
      var tx = wallet.createTransactionCapsule(tokenCtx, ContractType.AddNftMinterContract).getInstance();
