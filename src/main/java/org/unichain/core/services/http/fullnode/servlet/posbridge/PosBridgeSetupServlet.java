...
@Component
@Slf4j(topic = "API")
public class PosBridgeSetupServlet extends HttpServlet {
  @Autowired
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      var build = Contract.PosBridgeSetupContract.newBuilder();
      JsonFormat.merge(contract, build, visible);
      var tokenCtx = build.build();

      var tx = wallet.createTransactionCapsule(tokenCtx,
              Protocol.Transaction.Contract.ContractType.PosBridgeSetupContract).getInstance();
