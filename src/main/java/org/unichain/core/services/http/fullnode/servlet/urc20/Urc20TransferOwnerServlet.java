...
@Component
@Slf4j(topic = "API")
public class Urc20TransferOwnerServlet extends HttpServlet {
  @Autowired
  private Urc20 urc20;
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      var builder = Contract.Urc20TransferOwnerContract.newBuilder();
      JsonFormat.merge(contract, builder, visible);
      var tx = urc20.transferOwner(builder.build());
      var jsonObject = JSONObject.parseObject(contract);
      tx = Util.setTransactionPermissionId(jsonObject, tx);
      response.getWriter().println(Util.printCreateTransaction(tx, visible));
    } catch (Exception e) {
      try {
        logger.error("Urc20TransferOwner got error --> ", e);
