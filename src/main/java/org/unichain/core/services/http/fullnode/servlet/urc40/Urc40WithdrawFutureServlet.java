...
@Component
@Slf4j(topic = "API")
public class Urc40WithdrawFutureServlet extends HttpServlet {
  @Autowired
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      var build = Urc40WithdrawFutureContract.newBuilder();
      JsonFormat.merge(contract, build, visible);
      var withdrawCtx = build.build();
      var tx = wallet.createTransactionCapsule(withdrawCtx, ContractType.Urc40WithdrawFutureContract).getInstance();
      var jsonObject = JSONObject.parseObject(contract);
      tx = Util.setTransactionPermissionId(jsonObject, tx);
      response.getWriter().println(Util.printCreateTransaction(tx, visible));
    } catch (Exception e) {
      try {
        logger.error("Urc40WithdrawFuture got error --> ", e);
